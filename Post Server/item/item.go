...
func ViewById(w http.ResponseWriter, r *http.Request) {
	...
	defer config.CloseDBConnection(DB)
	var item Item
	check := DB.Table("item").Where("itemId =?", id).Take(&item)
	if check.Error != nil {
	...
	res := response.BuildResponse(true, "OK!", item)
	w.WriteHeader(http.StatusOK)
	response, _ := json.Marshal(res)
	w.Write(response)
	return
...
func Insert(w http.ResponseWriter, r *http.Request) {
	...
	result := DB.Exec("CALL insert_item(?)", string(marshalled))
	if result.Error != nil {
...
func Update(w http.ResponseWriter, r *http.Request) {
	...
	check := DB.Table("item").Where("itemId =?", item.ItemId).Take(&temp)
	if check.Error != nil {
	...
	marshalled, _ := json.Marshal(item)
	result := DB.Exec("CALL update_item(?)", string(marshalled))
	if result.Error != nil {
...
func Delete(w http.ResponseWriter, r *http.Request) {
	...
	defer config.CloseDBConnection(DB)
	var temp Item
	check := DB.Table("item").Where("itemId =?", id).Take(&temp)
	...
	result := DB.Exec("CALL delete_item(?)", id)
	if result.Error != nil {
		res := response.BuildErrorResponse("Cannot Insert Data", result.Error.Error(), response.EmptyObj{})
		w.WriteHeader(http.StatusBadRequest)
		response, _ := json.Marshal(res)
