...
type UomType struct {
	UomTypeId   int    `gorm:"column:uomTypeId;primary_key:auto_increament" json:"uomTypeId"`
	UomType     string `gorm:"column:uomType" json:"uomType"`
...
func ViewAll(w http.ResponseWriter, r *http.Request) {
	...
	var uomType []UomType
	result := DB.Raw("CALL viewAll_uomType()").Scan(&uomType)
	if result.Error != nil {
	...
	res := response.BuildResponse(true, "OK!", uomType)
...
func ViewById(w http.ResponseWriter, r *http.Request) {
	...
	defer config.CloseDBConnection(DB)
	var uomType UomType
	check := DB.Table("uomType").Where("uomTypeId =? ", id).Take(&uomType)
	if check.Error != nil {
	...
	result := DB.Raw("CALL view_uomType_byId(?)", id).Scan(&uomType)
	if result.Error != nil {
		res := response.BuildErrorResponse("No Data's Found", "No ID Found", response.EmptyObj{})
		w.WriteHeader(http.StatusBadRequest)
		response, _ := json.Marshal(res)
...
func Insert(w http.ResponseWriter, r *http.Request) {
	...
	var uomType UomType
	err := json.NewDecoder(r.Body).Decode(&uomType)
	if err != nil {
	...
	DB := config.SetupDBConnection()
	defer config.CloseDBConnection(DB)
	marshalled, _ := json.Marshal(uomType)
	result := DB.Exec("CALL insert_uomType(?)", string(marshalled))
	if result.Error != nil {
...
func Update(w http.ResponseWriter, r *http.Request) {
	...
	check := DB.Table("uomType").Where("uomTypeId =? ", uomType.UomTypeId).Take(&temp)
	if check.Error != nil {
	...
	marshalled, _ := json.Marshal(uomType)
	result := DB.Exec("CALL update_uomType(?)", string(marshalled))
	if result.Error != nil {
...
func Delete(w http.ResponseWriter, r *http.Request) {
	...
	defer config.CloseDBConnection(DB)
	var temp UomType
	check := DB.Table("uomType").Where("uomTypeId =? ", id).Take(&temp)
	...
	result := DB.Exec("CALL delete_uomType(?)", id)
	if result.Error != nil {
		res := response.BuildErrorResponse("Cannot Insert Data", result.Error.Error(), response.EmptyObj{})
		w.WriteHeader(http.StatusBadRequest)
		response, _ := json.Marshal(res)
