...
type ItemCategory struct {
	ItemCategoryId   int    `gorm:"column:itemCategoryId;primary_key:auto_increament" json:"itemCategoryId"`
	ItemTypeId       int    `gorm:"column:itemTypeId" json:"itemTypeId"`
	ItemCategoryCode string `gorm:"column:itemCategoryCode" json:"itemCategoryCode"`
...
func ViewAll(w http.ResponseWriter, r *http.Request) {
	...
	result := DB.Raw("CALL viewAll_itemCategory()").Scan(&itemCategory)
	if result.Error != nil {
	...
	res := response.BuildResponse(true, "OK!", itemCategory)
...
func ViewById(w http.ResponseWriter, r *http.Request) {
	...
	defer config.CloseDBConnection(DB)
	var itemCategory ItemCategory
	check := DB.Table("itemCategory").Where("itemCategoryId =? ", id).Take(&itemCategory)
	if check.Error != nil {
	...
	result := DB.Raw("CALL view_itemCategory_byId(?)", id).Scan(&itemCategory)
	if result.Error != nil {
		res := response.BuildErrorResponse("No Data's Found", "No ID Found", response.EmptyObj{})
		w.WriteHeader(http.StatusBadRequest)
		response, _ := json.Marshal(res)
...
func Insert(w http.ResponseWriter, r *http.Request) {
	...
	DB := config.SetupDBConnection()
	defer config.CloseDBConnection(DB)
	marshalled, _ := json.Marshal(itemCategory)
	result := DB.Exec("CALL insert_itemCategory(?)", string(marshalled))
	if result.Error != nil {
...
func Update(w http.ResponseWriter, r *http.Request) {
	...
	DB := config.SetupDBConnection()
	defer config.CloseDBConnection(DB)
	var itemCategory,temp ItemCategory
	err := json.NewDecoder(r.Body).Decode(&itemCategory)
	if err != nil {
		res := response.BuildErrorResponse("Failed to Process Request", err.Error(), response.EmptyObj{})
		w.WriteHeader(http.StatusBadRequest)
	...
	check := DB.Table("itemCategory").Where("itemCategoryId =? ", itemCategory.ItemCategoryId).Take(&temp)
	if check.Error != nil {
	...
	marshalled, _ := json.Marshal(itemCategory)
	result := DB.Exec("CALL update_itemCategory(?)", string(marshalled))
	if result.Error != nil {
...
func Delete(w http.ResponseWriter, r *http.Request) {
	...
	DB := config.SetupDBConnection()
	defer config.CloseDBConnection(DB)
	var temp ItemCategory
    check := DB.Table("itemCategory").Where("itemCategoryId =? ", id).Take(&temp)
	...
	result := DB.Exec("CALL delete_itemCategory(?)", id)
	if result.Error != nil {
		res := response.BuildErrorResponse("Cannot Insert Data", result.Error.Error(), response.EmptyObj{})
		w.WriteHeader(http.StatusBadRequest)
		response, _ := json.Marshal(res)
